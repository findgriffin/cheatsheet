...
Max power when:&&$R_L = R_{thevenin}$\\
...
reactive power & Q \\
complex power & S\\
apparent power &\textbar S\textbar\\
power factor & $\cos \psi$
\end{tabular}
$ Z_L = \jmath \omega L$ but 
$Z_C = \dfrac{-\jmath}{\omega C} = \dfrac{1}{\jmath \omega C}$
...
Half wave rectifier with capacitor. $V_r$ is the voltage drop after the capacitor has finished discharging. $T$ is the discharge time.
\[V_r = V_p \frac{T}{\tau} = V_p\frac{1}{f\tau }  \mbox{ \ \ \ \ \ where } \tau
= RC\]

Full wave rectifier.
\[V_r = \frac{V_p}{2f\tau } \mbox{ \ \ \ \ \ } f \mbox{ is the supply freq, }
\frac{1}{2} \mbox{ new freq}\]
...
When $v_{GS} \ge V_t$ the channel is conductive. $L$ is length between $S$ and
$D$ doped regions (0.1-3$\mu$m). $W$ is width of device (0.2-100$\mu$m).
